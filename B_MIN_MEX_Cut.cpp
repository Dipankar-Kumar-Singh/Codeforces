...
signed main()
{
    ...
    t_case
    {
        ...
        int fregmented_zero = 0 ;
        f(x,mini){ if(x=='0'){fregmented_zero++;}}
        ...
        f(x,mini){
            if(x=='0'){hash_012_[0]++ ;}
            else if(x=='1'){hash_012_[1]++ ;}
        }

        set<int> not_founds ;

        fl(0,2){ int x = hash_012_[i] ; if (x ==-1){ not_founds.insert(i) ;}}
 
        int MEX = *not_founds.begin() ;

       if(MEX < 2 ){ cout << MEX << "\n" ; continue ;}

       if(fregmented_zero<MEX){ cout << fregmented_zero << "\n" ; continue ;}
       else {cout << MEX << "\n" ; continue ; }
