...
int isPossible(int val, int i ){
    int x = (i - val );
    return  (x <= k) ;
}

int last(int val , int l , int r ){
    int ans = -1 ;
    while(l<=r){
        int m = l + (r-l) /2 ;
        if(isPossible(val,v[m])){
            ans = m ;
            l = m+1 ;
        }
        else { r = m -1 ;}
...
signed main()
{
    FAST 
    ll n ; cin >> n >> k;
    fl(0,n-1){int x ; cin >> x  ; v.pb(x) ;}
    vi pool_size_store ;

    for(int i = 0 ; i <= n-3 ; i++)
    {               
        int x = last(v[i],i+1,n-1) ;
        int pool_size = x - i  ; 
        pool_size_store.pb(pool_size);
    }
