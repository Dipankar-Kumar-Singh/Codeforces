...
void cswap(vvi &v, int a , int b , int n , int m ){
    ff(n){
        swap(v[i][a], v[i][b]);
    }
}

void rswap(vvi &v, int a , int b , int n , int m ){
...
signed main()
{
    ...
    while(k--){
        ...
        if     (c == 'c'){ cswap(v, a-1 ,  b-1 , n ,m) ;     }
        else if(c == 'r'){ rswap(v, a -1 , b -1  , n ,m) ;     }
        else if(c == 'g'){ cout << v[a-1][b-1] << endl ; }
