...
int change(int a, int b , int u){
    if(u&1){ return a ; } else { return b ; }
}
...
signed main()
{
    ...
    t_case
    {
        ...
        if(h-a[0] <= 0 ){ cout << 1 << endl; continue ;}
        else if(h-s<=0){ cout << 2 << endl; continue ;}
       ...
       if(h>0)
       {
            if(h-a[0] <= 0){cout << (ans)+1 << endl; continue ;}
            else { cout << ans+ 2 << endl; continue ; } 
