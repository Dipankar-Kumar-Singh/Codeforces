...
{   
    ...
    f(x,s){m[x]++ ;} // frequency_maper

    ll even = 0 , odd = 0 ;
    ll single_occurance = 0 ;
    ll more_than_one_occurance = 0;
    f(x,m)
    {
        if(x.second == 1){ single_occurance++ ;}

        else if(x.second==2){ ans++ ;}
        else if(x.second == 3) {ans++;}
        else { ans++ ;}


    }

    if(single_occurance%2==0){single_occurance =single_occurance/2 ;}
    else { single_occurance = ((single_occurance-1)/2) ;}

    ans += single_occurance  ;
