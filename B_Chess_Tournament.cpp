...
void put_val_zero(int x , vvi &m , int n){
...
signed main()
{
    ...
    {      
        ...
        vvi m (n+1 ,vi (n+1,empty1)) ;

        fl(1,n) m[i][i] = -100 ;

        vi winer_count (n+1,0) ;

        //* NOTE Fixing the value 
        ...
        fl(1,n){
           cfl(1,n,j){
               if(m[i][j]==empty1 and winer_count[i]==0){
                   winer_count[i]++ ;
                   m[i][j] = win ; 
                   m[j][i] = lose ;
               }

               else if(m[i][j]==empty1 and !winer_count[i]==0){
        ...
        // WR("WINNER COUNT : ") ; print(winer_count) ;
        // WR("S = ") ; print(s) ;
        // print_2D(m) ; nl nl 
        // print(winer_count) ;
        ...
        int posible = true ;
        fl(0,n-1){
            if(s[i]=='2' and  winer_count[i+1]<=0){
              posible = false ;
            //   cout << " (" << i << ") " ; break ;
            //   cout << "winner c = " << winer_count[i+1] << "\n" ; 
            }
        }
       // error(empty1) ;
        if(posible)
        {
            ...
            fl(1,n) {
                cfl(1,n,j){
                    int val = m[i][j] ;
                    if( val == -100){ cout << "X" ;}
                    else if(m[i][j]==win){cout << '+' ;}
                    else if(m[i][j]==lose){cout << '-' ; }
                    else if(m[i][j]==draw){cout << '=' ;}
                } nl 
            }
        }
