...
signed main()
{
    ...
    vi remaning ;
    f(x,m){if(x.S==0){ remaning.pb(x.F);}}

    multiset<int> toBeChanged ;
    for(auto &[x,y] : m){
        if(y > 1 or x<=0 or x>n)
        {   
            bool outOFBound = (x<=0 or x>n);

            if(outOFBound){
                ff(y){toBeChanged.insert(x) ;} 
            }

            else {
                ff(y-1){toBeChanged.insert(x) ;} 
            }
        }
    }

    // dbg(toBeChanged,remaning,m) ;

    int ans = 0 ;

    f(x,remaning){
        
        auto it = toBeChanged.lower_bound(x) ;

        // dbg(remaning , toBeChanged,x,*it) ;

        // !! Possible Error due to -x ==> ( - * -)  => + 

        

        if(  it != toBeChanged.begin()  )
        {
            auto it2 = it ;
            --it2 ; int distance = INFLL , distance2 = abs((*it2) - x)  ;

            if(it != toBeChanged.end()){ 
                int distance = abs((*it) - x) ;
            }
            ...
            if(distance2 <= distance    )
            {
                ...
                toBeChanged.erase(it2) ;
                ans+=distance;

                // dbg(distance2,distance,toBeChanged) ;
                // cerr << "1-------------------\n" ;
                // dbg(toBeChanged ) ;
                // cerr << "11-------------------\n" ;

            }
            else if( distance2 > distance  and it != toBeChanged.end()) 
            { 
                ans+=distance;  toBeChanged.erase(it) ; 

                // dbg(distance2,distance)
                // cerr << "2-------------------\n" ;
                // dbg(toBeChanged)
                // cerr << "22-------------------\n" ;
            }
        ...
        else {      int distance = INFLL ;
                    distance = abs(*it -x) ;
                    if(it != toBeChanged.end())
                    {   
                        ans += distance;
                        toBeChanged.erase(it) ; 
                        // cerr << "#######################\n" ;
                        // dbg(toBeChanged)
                        // cerr << "#######################\n" ;
                    }
