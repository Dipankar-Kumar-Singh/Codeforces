...
{ 
    ...
    while(l<=r){ 
        ...
        if(v[mid]>t){r=mid-1;} else l = mid+1;
    }

    return -1 ;
}

void check(int &i , int &current,vi &v,vi &sv, int &changes,int n)
{   changes++ ; 
    int j = bn(sv,n,current);
    for(i; i<n ; i++ , j++)  {if(v[i]!=sv[j]) {break ;}}
}
...
signed main()
{
    ...
    t_case
    {
        ...
        fl(0,n-1)
        {
            check(i,v[i],v,sv,changes,n) ; i-- ; 
        } 

        if(changes<=k){ cout << "Yes\n" ;} else cout << "No\n" ;
