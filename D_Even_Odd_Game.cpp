...
signed main()
{
    ...
    t_case
    {
        ...
        int n ; cin >> n ; tv(n,v,ll) ;

        vi O , E ;   int odd {} , even {} ; 

        f(x,v){if(x&1){O.eb(x) ; odd++ ; } else{ E.eb(x) ; even++ ;}}

        int nMin = min(odd,even) ;  
        ...
        if(n==1 and (v[0]&1)){ cout << "Tie\n" ; continue ; }
        else if(n==1 and v[0]%2==0)
        { 
            if(v[0]>0){ cout << "Alice\n" ; continue ; }
            else cout << "Tie\n" ; continue ;  ; 
        } 
