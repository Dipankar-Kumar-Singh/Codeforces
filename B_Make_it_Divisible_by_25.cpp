...
int f25(str s){ 
    ...
    fl(0,n-1){
        if(s[i]=='5'){id5 = i ; break ; } 
    }

    fl(id5+1,n-1){ if(s[i] == '2'){id2 = i ;  break ;}}

    if(id5 == -1 or id2==-1){ return INFLL ; }

    int ans = 0 ; ans+=id5;
    ans += (id2-id5-1 ); 

    
    // error( s , "f25" ,id2 , id5  , ans) ; 
...
int f50(str s)
{
    ...
    fl(0,n-1){
        if(s[i]=='0'){id0 = i ; break ; } 
    }

    fl(id0 + 1,n-1){ if(s[i] == '5'){id5 = i ;  break ;}}

    if((id5 == -1) or (id0==-1)){ return INFLL ; }
...
int f75(str s){
    ...
    fl(id5+1,n-1){ if(s[i] == '7'){id7 = i ;  break ;}}

    if((id5 == -1) or (id7==-1)){ return INFLL ; }
...
int f100(str s){
    ...
    fl(0,n-1){
        if(s[i]=='0'){id01 = i ; break ; } 
    }

    fl(id01+1,n-1){
        if(s[i] =='0'){id02 = i ; break ; } 
    }

    // fl(id02+1,n-1){ if(s[i] != '0'){ idnum  = i ;  break ;}}

    // error( s , "f100" ,idnum , id02 , id01  ) ; 

    if(  (id01 == -1) or (id02 == -1)){ return INFLL ; }

    int ans = 0 ; 
    ans += id01;
    ans += (id02 - id01 -1 ) ;  

    // error( s , "f100" ,idnum , id02 , id01  , ans) ; 

    bool cond = false ;
    fl(id01,id02){if(s[i]!='0') cond = true ;}

    // if(cond){return ans -1 ;}
...
signed main()
{
    ...
    t_case
    {
        ...
        // int a = f25(s), b = f50(s)  , c = f75(s) , d = f100(s) ;

        // error(a,b,c,d) ; cerr << "\n\n" ;

        // if(stoll(s)%25==0){ cout << 0 << "\n" ; continue ;}
        cout << min({f25(s),f50(s),f75(s) , f100(s)}) << "\n"  ; 
