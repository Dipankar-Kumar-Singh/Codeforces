...
{    // 1 --> ORG , //2 --> srt
    ...
    t_case
    {
       ...
       // error(odd , even)
        ...
        if(even==(odd+1))
        {    srt.pb(0) ;
            fl(1,n-1){ if(i&1)srt.eb(1) ; else srt.eb(0) ; }
        }
        else if(odd==(even+1))
        {    srt.pb(1) ;
            fl(1,n-1){ if(i&1)srt.eb(0) ; else srt.eb(1) ; }   
        }
        else if(odd == even)
        {   srt.pb(org[0]) ;
            fl(1,n-1){ if(i&1)srt.eb(!org[0]) ; else srt.eb(org[0]) ; }
        }

       // cout << "SRT : " ; print(srt) ;  cout << "ORG : " ;  print(org) ;   nl ;
        // construning ideal arr is done //
        // now start comparing the mismatch value 

        fl(0,n-1)
        {   int target = srt[0] ;
            int a = org[i] , b = srt[i] ;
            if(a==target){ s1.insert(i) ;}
            if(b==target){ s2.insert(i) ;}
        }
        //  cout << "S1 : " ;  print(s1) ;  cout << "S2 : " ; print(s2) ;
        // s1 [a]pos --- 2  4  6    org pos 
        // s2 [a]pos --- 4  5  6    optimal pos
        // Bug --> don't delete elemt from 1 ; 
        //  vector of uniue pos //
        vi v1 , v2 ;

        f(x,s2)
        {   if(cpresent(s1,x))   {  s1.erase(x) ;   }
            else {v2.eb(x) ;}
        }
        // v1 will not have only elemts which are not common with s2 !(s1 inteersextion s2)
        f(x,s1) {   v1.eb(x) ;   }
       
        //   cout << " V1 : " ; print(v1)  ;   cout << " V2 : " ;print(v2)  nl
        //  f(x,s2) {   v2.eb(x) ;   } // no need as we are making vec above only
