...
signed main()
{
    ...
    {   
        ...
        int max_posible_a  = a -1 ;
        bool touching_zero = false ;
        bool only_one_type = b==0 or c == 0 ;
        int mx = max(a-1,0LL) + max(b-1,0LL) + max(c-1,0LL)  ;
        bool under_H_limit = (p <= mx) ; 

        if(!under_H_limit){cout << "NO\n" ; continue ; }
        ...
        if(a-1 <= x){
            ...
            int &y = max_posible_a ;
            int minima = max(y-x,0LL) ;

            if(p>=minima){ cout << "YES\n"  ; continue ; }
            else {cout << "NO\n" ; continue ; }
        }
