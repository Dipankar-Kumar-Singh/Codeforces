...
#define take_vector(size, vector_name, type) \
    vector<type> vector_name(size);          \
    fl(0LL, size - 1) { cin >> vector_name[i]; }
#define tv(size, vector_name, type) \
    vector<type> vector_name(size); \
    fl(0LL, size - 1) { cin >> vector_name[i]; }
#define take_set(from, set_name, type) \
    set<type> set_name;                \
    for (auto &x : from)               \
    {                                  \
        set_name.insert(x);            \
    }
#define frequency_maper(from, map_name, type) \
    map<type, ll> map_name;                   \
    f(x, from) { map_name[x]++; }
#define hash_alpha_Lower(string_name, hash_name) \
    vi hash_name(27, 0);                         \
    f(x, string_name) { hash_name[(x - 'a' + 1)]++; }
#define hash_alpha_Upper(string_name, hash_name) \
    vi hash_name(27, 0);                         \
    f(x, string_name) { hash_name[(x - 'A' + 1)]++; }
...
#define print_map(map_name)                                                                 \
    f(elements, map_name) { cout << elements.first << " --> " << elements.second << "\n"; } \
    cout << "\n";
#define print_2D(vector_name)         \
    f(x, vector_name)                 \
    {                                 \
        f(y, x) { cout << y << " "; } \
        cout << "\n";                 \
...
void write_error() {}
template <typename T, typename... Args>
void write_error(T &t, Args &...args)
{
    cerr << t << " ";
    write_error(args...);
}
...
signed main()
{
    ...
    if((((up&1) and !(down&1)) ) or ((down&1) and !(up&1)))
    {
        cout << -1 ; return 0 ; 
    }
