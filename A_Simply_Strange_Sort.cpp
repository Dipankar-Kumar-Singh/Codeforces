 ...
 void solve (int &n , int &ans , vi &v) 
{
   ...
   // print(v) ;
    ...
    for(int i = 1 ; i <= n ; i++)
    {
        if(v!=s)
        {
            if(i&1){ odd_call(n,v) ; ans++ ;}
            else{ even_call(n,v) ; ans++ ;}
        }
...
signed main()
{
    ...
    {
        int n ; cin >> n ; 
        vi v(1,-10000000000000) ; 
        fl(0,n-1){ int x ; cin >> x ; v.eb(x) ;}
